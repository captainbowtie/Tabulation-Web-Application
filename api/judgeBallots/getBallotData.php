...
/*
 Determines which ballot to pull in priority order:
 1. If specific ballot is identified, get that ballot
 2. If only pairing is identified, get that judge's ballot for that pairing
 3. If only pairing is identified, but ballot doesn't exist, create a new one
 */
if (isset($_GET["ballot"])) { //if the user has a specific ballot in mind, get that ballot
	$ballotURL = htmlspecialchars(strip_tags($_GET["ballot"]));
	$ballot = getBallotDataFromURL($ballotURL);
	...
	if ($ballot["locked"]) {
		...
		$_SESSION["ballot"] = $ballot["url"];

		echo json_encode($ballotData);
	}
} elseif (isset($_GET["pairing"])) { //if a specific ballot is not identified, go to pairing instead

	$pairingURL = htmlspecialchars(strip_tags($_GET["pairing"]));
	if (isset($_SESSION["ballot"])) { //check if the judge already has a ballot for the pairing
		$ballotURL = htmlspecialchars(strip_tags($_SESSION["ballot"]));
		$ballot = getBallotDataFromURL($ballotURL);
		$pairing = getPairingDataFromId($ballot["pairing"]);
		$pTeamNumber = getTeamNumberFromId($pairing["plaintiff"]);
		$dTeamNumber = getTeamNumberFromId($pairing["defense"]);
	...
	} else { //if judge doesn't have existing ballot for the pairing, create one
		$pairingURL = htmlspecialchars(strip_tags($_GET["pairing"]));

		$ballotURL = bin2hex(random_bytes(32));
		$_SESSION["ballot"] = $ballotURL;
		$pairing = getPairingDataFromURL($pairingURL);
		$pTeamNumber = getTeamNumberFromId($pairing["plaintiff"]);
		$dTeamNumber = getTeamNumberFromId($pairing["defense"]);

		createBallot($pairing["id"], $ballotURL);
...
} else { //if neither ballot nor pairing is identified, give up
	die();
}

function createBallot($pairingId, $url)
{
	try {
		$db = new Database();
		$conn = $db->getConnection();
		$ballotStmt = $conn->prepare("INSERT INTO ballots (pairing,url) VALUES (:pairingId,:url)");
		$ballotStmt->bindParam(':url', $url);
		$ballotStmt->bindParam(':pairingId', $pairingId);
		$ballotStmt->execute();
		$conn = null;
...
function getBallotDataFromURL($ballotURL)
{
	try {
		$db = new Database();
		$conn = $db->getConnection();
		$ballotStmt = $conn->prepare("SELECT * FROM ballots WHERE url=:url");
		$ballotStmt->bindParam(':url', $ballotURL);
		$ballotStmt->execute();
